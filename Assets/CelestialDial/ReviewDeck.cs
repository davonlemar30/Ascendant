...
namespace Ascendant.CelestialDial
{
    ...
    public enum ItemKind { Element, Glyph, Modality, Grid, Opposite } // v0.3 adds twelve glyph items; Build A twelve sign ↔ modality items; Build B twelve sign → cell items; Build C six opposite pairs (Curriculum canon Stage 1).
    ...
    public sealed class ReviewDeck
    {
        ...
        public void IntroduceAll(int day, ItemKind kind) { IntroduceAll(day, kind, 12); }
        public void IntroduceAll(int day, ItemKind kind, int count) // the opposite kind has six pair items, seats 0–5
        {
            foreach (var item in Items) if (item.Kind == kind && item.seat < count && !item.entered) { item.entered = true; item.interval = 0; item.dueDay = day; } // ready at the next check
            Logged?.Invoke(kind == ItemKind.Element ? "deck_introduced" : kind == ItemKind.Glyph ? "deck_glyphs_introduced" : kind == ItemKind.Modality ? "deck_modalities_introduced" : kind == ItemKind.Grid ? "deck_grid_introduced" : "deck_opposites_introduced");
        }
    ...
    public sealed class SaveData
    {
        ...
        public bool polarityShown;               // Build C: the last pattern and the builder; keys carries Key 4
        public bool[] oppKnown = new bool[6];
