...
namespace Ascendant.Build
{
    ...
    public static class GreyboxValidation
    {
        ...
        public static void Run()
        {
            ...
            Check(Slots.Art.Length==33 && Slots.Art.Select(a=>a.Name).Distinct().Count()==33 && Slots.Art.All(a=>a.Name.All(c=>char.IsLower(c)||c=='-') && a.Width>0 && a.Height>0 && a.MaxSize>=Mathf.Max(a.Width,a.Height) && a.Where.Length>0),"33 art slots with unique kebab-case names, a rect, a size cap at or above the rect, and a place");
            ...
            Check(Slots.Set=="test" && Slots.Art.All(a=>Slots.Image(a.Name)!=null) && Slots.Sounds.All(s=>Slots.Clip(s.Name)!=null) && Slots.ArtFiles==33 && Slots.SoundFiles==7,"the shipped test set has a file for every slot and the loader finds each one");
            ...
            Check(atriumImporter!=null && atriumImporter.textureType==TextureImporterType.Sprite && !atriumImporter.mipmapEnabled && atriumImporter.crunchedCompression==!atriumImporter.DoesSourceTextureHaveAlpha() && atriumImporter.textureCompression==TextureImporterCompression.Compressed && atriumImporter.maxTextureSize==2048 && lockImporter!=null && lockImporter.maxTextureSize==32,"a PNG in the slot folder imports as a sprite: no mipmaps, crunched unless it has transparency, the slot's size cap");
