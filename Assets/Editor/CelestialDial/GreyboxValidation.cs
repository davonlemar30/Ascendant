...
namespace Ascendant.Build
{
    ...
    public static class GreyboxValidation
    {
        ...
        public static void Run()
        {
            ...
            var gd=new ReviewDeck();gd.IntroduceAll(0);gd.IntroduceAll(0,ItemKind.Glyph);Check(gd.Items.Length==48 && gd.Due(1).Count==24 && gd.Item(3,ItemKind.Glyph).Kind==ItemKind.Glyph && gd.Item(3,ItemKind.Modality).Kind==ItemKind.Modality && !gd.Item(3,ItemKind.Modality).entered && !gd.Item(3,ItemKind.Grid).entered,"the deck holds twelve element, glyph, modality, and grid items; the modality and grid items wait");
            ...
            Check(Rooms.Visible(Room.Atrium).Count(p=>p.Walkable)==3 && Rooms.Visible(Room.Atrium).Count(p=>!p.Walkable)==2 && Rooms.Visible(Room.Wing).Count()==4 && Rooms.Visible(Room.Wing).All(p=>p.Walkable) && Rooms.Find(Room.Wing,"grid").X==-60 && Rooms.Find(Room.Wing,"grid").Label=="the table","the Atrium offers the doorway, the desk, and Caspar plus two sealed doors; the Wing offers the doorway back, the table, the Dial, and the shelf");
            ...
            Check(jsave.deck!=null && jsave.deck.Length==48 && jback.Restore(jsave) && jback.DueCount==jflow.DueCount && jback.Deck.Practicing==1,"the review deck survives the JSON save and reload with its due count and practicing items");
            ...
            Check(tflow.LeaveWing() && tflow.AtriumStage==4,"two Keys: the Atrium stays at Stage 4");
            ...
            tflow.MarkKey3();Check(tflow.Keys==3 && tflow.EnterWing() && tflow.LeaveWing() && tflow.AtriumStage==5,"Key 3 and one more return take the Atrium to Stage 5");
            ...
            Check(tsave.deck.Length==48 && tsave.gridPlaced.All(v=>v) && tsave.gridEvidence && tsave.gridStarted && tback.Restore(tsave) && tback.Keys==3 && tback.AtriumStage==5 && tback.ModalitiesComplete && tback.GridStarted && tback.Deck.Item(4,ItemKind.Grid).State==ItemState.Practicing && tback.DueCount==0 && tback.EnterWing() && tback.CanOpenGrid,"the save carries the table, Key 3, Stage 5, and the grid items as data");
