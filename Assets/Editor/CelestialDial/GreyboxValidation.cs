...
namespace Ascendant.Build
{
    ...
    public static class GreyboxValidation
    {
        ...
        public static void Run()
        {
            ...
            var gd=new ReviewDeck();gd.IntroduceAll(0);gd.IntroduceAll(0,ItemKind.Glyph);Check(gd.Items.Length==36 && gd.Due(1).Count==24 && gd.Item(3,ItemKind.Glyph).Kind==ItemKind.Glyph && gd.Item(3,ItemKind.Modality).Kind==ItemKind.Modality && !gd.Item(3,ItemKind.Modality).entered,"the deck holds twelve element, twelve glyph, and twelve modality items; the modality items wait");
            ...
            Check(jsave.deck!=null && jsave.deck.Length==36 && jback.Restore(jsave) && jback.DueCount==jflow.DueCount && jback.Deck.Practicing==1,"the review deck survives the JSON save and reload with its due count and practicing items");
