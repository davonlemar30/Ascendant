...
namespace Ascendant.Build
{
    ...
    public static class GreyboxValidation
    {
        ...
        static void ValidateBuildF()
        {
            ...
            Check(f.OpenJournal() && f.AtJournal && f.JournalContents && f.JournalFrom == SliceScreen.WingRoom && f.JournalSections.SequenceEqual(new[] { ItemKind.Element, ItemKind.Glyph }) && f.JournalEntries(ItemKind.Element).Count == 12 && f.JournalEntries(ItemKind.Element)[0] == "Aries — Fire" && !f.JournalEntries(ItemKind.Element).Any(e => e.Contains("introduced") || e.Contains("practicing")), "the journal opens to a contents page with only unlocked sections and no state words");
            Check(f.JournalOpenSection(ItemKind.Glyph) && !f.JournalContents && f.JournalKind == ItemKind.Glyph && f.JournalEntries(ItemKind.Glyph).Count == 12 && f.JournalOpenSection(ItemKind.Element) && f.JournalKind == ItemKind.Element && f.JournalContentsPage(), "contents entries open their section pages and the Contents control returns");
            Check(f.JournalOpenSigns() && f.JournalSigns && f.LearnedSignSeats.Count == 12 && f.JournalSignLearned(3, ItemKind.Element) && !f.JournalSignLearned(3, ItemKind.Modality), "The Signs lists only met signs and a sign page exposes only learned facts");
        ...
        public static void Run()
        {
            ...
            Check(Slots.Art.Length==48 && Slots.Art.Select(a=>a.Name).Distinct().Count()==48 && Slots.Art.All(a=>a.Name.All(c=>char.IsLower(c)||c=='-') && a.Width>0 && a.Height>0 && a.MaxSize>=Mathf.Max(a.Width,a.Height) && a.Where.Length>0),"48 art slots with unique kebab-case names, a rect, a size cap at or above the rect, and a place");
            ...
            Check(Slots.Set=="test" && Slots.Art.All(a=>Slots.Image(a.Name)!=null) && Slots.Sounds.All(s=>Slots.Clip(s.Name)!=null) && Slots.ArtFiles==48 && Slots.SoundFiles==7,"the shipped test set has a file for every slot and the loader finds each one");
