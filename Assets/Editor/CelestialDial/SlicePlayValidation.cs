...
namespace Ascendant.Build
{
    ...
    public static class SlicePlayValidation
    {
        ...
        static void QueueChecks()
        {
            ...
            Steps.Enqueue(()=>{var g=View.Grid;Check(g.Complete && g.Key3Earned && g.Phase==GridPhase.Complete && View.Flow.Keys==3 && g.Events.Count(e=>e.event_name=="key3_earned")==1 && !g.Active && View.LastCeremony==3,"twelve seated earns Key 3 once, with its Key ceremony on the table (Build I)");Check(GameObject.Find("Table Keeper Key")==null,"the game is idle only once the Key 3 ceremony is over (it follows the seating; Sept 25)");Capture("slice-390-grid-key3.png");});
