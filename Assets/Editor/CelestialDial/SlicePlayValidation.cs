...
namespace Ascendant.Build
{
    ...
    public static class SlicePlayValidation
    {
        ...
        static void QueueChecks()
        {
            ...
            Steps.Enqueue(()=>{Check(View.Flow.Screen==SliceScreen.Hub && !View.Busy && View.Flow.AtriumStage==5 && View.Flow.Keys==3,"one more return takes the Atrium to Stage 5 with three Keys");Capture("slice-390-hub-key3.png");});
            Steps.Enqueue(()=>Act("reload"));
            Steps.Enqueue(()=>{Check(View!=null && View.Resumed && View.Flow.Screen==SliceScreen.Hub && View.Flow.WheelComplete && View.Flow.AtriumStage==5 && View.Flow.Keys==3 && View.Dial.Lesson.Key2Earned && View.Grid.Key3Earned && View.Grid.Complete && View.Flow.CleanRuns==1 && View.Dial.Lesson.Hard && View.Flow.Walk.At=="entry","a reload resumes at the Hub from the local save with three Keys and the full table");Act("enter-wing");});
            ...
            Steps.Enqueue(()=>Act("leave-wing"));
            Steps.Enqueue(()=>{Check(View.Flow.Screen==SliceScreen.Hub && !View.Busy && View.Flow.AtriumStage==6 && View.Flow.Keys==4,"one more return takes the Atrium to Stage 6 with four Keys");Capture("slice-390-hub-key4.png");});
            Steps.Enqueue(()=>Act("reload"));
            Steps.Enqueue(()=>{Check(View!=null && View.Resumed && View.Flow.Screen==SliceScreen.Hub && View.Flow.AtriumStage==6 && View.Flow.Keys==4 && View.Dial.Lesson.Key4Earned && View.Dial.Lesson.PolarityShown && View.Dial.Lesson.OppositesComplete && View.Flow.Deck.Items.Count(i=>i.Kind==ItemKind.Opposite && i.entered)==6 && View.Grid.Key3Earned && View.Flow.Walk.At=="entry","a reload resumes at the Hub from the local save with four Keys, the sides, the six pairs, and the pair items as data");Act("restart");});
