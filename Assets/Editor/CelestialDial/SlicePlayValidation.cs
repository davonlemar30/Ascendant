...
namespace Ascendant.Build
{
    ...
    public static class SlicePlayValidation
    {
        ...
        static void QueueChecks()
        {
            ...
            // the Atrium's taps (Sept 25 fix): a comment had swallowed the doors' Tappable calls since Sept 24, and an invisible target with its mesh culled takes no tap
            Steps.Enqueue(()=>{var doors=new[]{"Sealed","Zodiac Wing, open","Crystal Book Chamber"}.Select(n=>GameObject.Find(n)).ToList();Check(View.Flow.Screen==SliceScreen.Hub && doors.All(d=>d!=null && d.GetComponent<UnityEngine.UI.Button>()!=null),"each Atrium door is a tap target");
                var invisible=UnityEngine.Object.FindObjectsByType<UnityEngine.UI.Button>(FindObjectsSortMode.None).Select(b=>b.GetComponent<UnityEngine.UI.Image>()).Where(i=>i!=null && i.color.a==0).ToList();
                Check(invisible.Count>=3 && invisible.All(i=>!i.canvasRenderer.cullTransparentMesh),"every invisible tap target keeps its mesh, so the raycaster sees it ("+invisible.Count+" on the Atrium)");});
            ...
            Steps.Enqueue(()=>{Check(View!=null && View.Resumed && View.Flow.Screen==SliceScreen.Hub && Slots.Set==Slots.TestSet && Slots.ArtFiles==109 && Slots.SoundFiles==7,"?art=test: the save resumes at the Hub with the test set, every slot with a file");
            ...
            Steps.Enqueue(()=>{var s=View.Dial.Snapshot();Check(View.StyleShown && s.screen=="style" && s.style && s.styleSlots.Length==109 && s.styleSounds.Length==7 && s.styleSlots.All(t=>t.EndsWith(": test set")) && s.styleSounds.All(t=>t.EndsWith(": test set")) && s.artSet==Slots.TestSet,"?style=test: the style page lists every slot on the test set, each with its source");Capture("slice-390-style-test.png");});
            ...
            Steps.Enqueue(()=>{var s=View.Dial.Snapshot();Check(View.StyleShown && s.styleSlots.Length==109 && s.styleSounds.Length==7 && s.styleSlots.All(t=>t.EndsWith(": file") || t.EndsWith(": placeholder")) && s.styleSounds.All(t=>t.EndsWith(": file") || t.EndsWith(": silent")) && s.artSet=="" && (Slots.DressedCount==0 || Slots.ArtFiles>0),"?style: the style page on the Art folder lists every slot as file or placeholder; nothing is dressed without a file");Capture("slice-390-style.png");});
