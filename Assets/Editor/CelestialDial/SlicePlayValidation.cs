...
namespace Ascendant.Build
{
    ...
    public static class SlicePlayValidation
    {
        ...
        static void QueueChecks()
        {
            ...
            Steps.Enqueue(()=>{Check(View!=null && View.Resumed && View.Flow.Screen==SliceScreen.Hub && Slots.Set==Slots.TestSet && Slots.ArtFiles==33 && Slots.SoundFiles==7,"?art=test: the save resumes at the Hub with the test set, every slot with a file");
                Check(new[]{"atrium","atrium-light","caspar","lamp","door-sealed","door-open","desk","shelves","shelf-book","keeper-idle"}.All(Slots.IsDressed) && View.LightAlpha==SliceView.LightAlphaFor(View.Flow.AtriumStage) && View.LightAlpha>0,"the Atrium takes its files: background, Caspar, lamps, doors, desk, shelves and their books, the Keeper");Capture("slice-360-art-hub.png");Debug.Log("[SlicePlayValidation] ambient loop playing: "+Sound.AmbientPlaying);});
            ...
            Steps.Enqueue(()=>{Check(View.Flow.Screen==SliceScreen.WingRoom && !View.Busy && Sound.LastCue=="door" && Sound.Played>playedBefore,"the doorway plays the door cue from the test set (cues played: "+Sound.Played+")");
                Check(new[]{"wing","wing-light","shelf","table","dial-face"}.All(Slots.IsDressed) && (Slots.IsDressed("keeper-idle") || Slots.IsDressed("keeper-walk")),"the Wing room takes its files: background, shelf, table, the Dial's face, the Keeper");Capture("slice-360-art-wing-room.png");Act("walk:dial");});
            ...
            Steps.Enqueue(()=>{Check(View.Flow.Screen==SliceScreen.ChamberRoom && new[]{"chamber","chamber-light","crystal-book","crystal-page","lock","mechanism","candle","door-open"}.All(Slots.IsDressed),"the Chamber takes its files: background, Books, pages, locks, mechanism, candles, doorway");Capture("slice-360-art-chamber.png");});
            ...
            Steps.Enqueue(()=>{var s=View.Dial.Snapshot();Check(View.StyleShown && s.screen=="style" && s.style && s.styleSlots.Length==33 && s.styleSounds.Length==7 && s.styleSlots.All(t=>t.EndsWith(": test set")) && s.styleSounds.All(t=>t.EndsWith(": test set")) && s.artSet==Slots.TestSet,"?style=test: the style page lists every slot on the test set, each with its source");Capture("slice-390-style-test.png");});
            ...
            Steps.Enqueue(()=>{var s=View.Dial.Snapshot();Check(View.StyleShown && s.styleSlots.Length==33 && s.styleSounds.Length==7 && s.styleSlots.All(t=>t.EndsWith(": file") || t.EndsWith(": placeholder")) && s.styleSounds.All(t=>t.EndsWith(": file") || t.EndsWith(": silent")) && s.artSet=="" && (Slots.DressedCount==0 || Slots.ArtFiles>0),"?style: the style page on the Art folder lists every slot as file or placeholder; nothing is dressed without a file");Capture("slice-390-style.png");});
