...
namespace Ascendant.Build
{
    ...
    public static class SlicePlayValidation
    {
        ...
        static void QueueChecks()
        {
            ...
            // ---- v0.2: the return ----
            Steps.Enqueue(()=>{Act("next-screen");Check(View.Flow.Screen==SliceScreen.Hub && View.Flow.AtriumStage==2 && View.Flow.DueCount==0 && UnityEngine.PlayerPrefs.HasKey(SliceView.SaveKey),"the Chamber leads to the Hub in Stage 2 with a save written");Check(View.Flow.Walk.Room==Room.Atrium && View.Flow.Walk.At=="entry","the marker stands where you came in");Capture("slice-390-hub.png");Act("walk:sealed-left");Check(View.Flow.Note.StartsWith("Sealed") && !View.Busy,"a sealed door only says it is sealed");});
            Steps.Enqueue(()=>{Act("enter-seals");Check(View.Flow.Walk.TargetId=="desk","Check the Seals walks to the desk first");});
            Steps.Enqueue(()=>{Check(View.Flow.Screen==SliceScreen.Hub && View.Flow.Walk.At=="desk" && View.Flow.Note.Contains("Nothing is due"),"nothing is due on the first sitting");Act("advance-day");Check(View.Flow.DueCount==12,"advance one day (test) makes the deck due");Act("enter-seals");});
            Steps.Enqueue(()=>{Check(View.Flow.Screen==SliceScreen.Review && View.Flow.ReviewQueue.Count==6 && View.Dial.Lesson.Phase==LessonPhase.Review && View.Dial.UiCanvas.gameObject.activeSelf,"Check the Seals opens with a compressed Dial item");Capture("slice-390-review-dial.png");});
            ...
            Steps.Enqueue(()=>{Check(View.Flow.Screen==SliceScreen.WingRoom && !View.Busy,"the Wing button walks through the same doorway");Act("walk:dial");});
            Steps.Enqueue(()=>{Check(View.Flow.Screen==SliceScreen.Wing && View.Dial.Lesson.Phase==LessonPhase.GlyphNames && View.Flow.GlyphsStarted && View.Flow.Deck.Items.Count(i=>i.Kind==ItemKind.Glyph && i.entered)==12,"the lit Wing opens the glyph unit, Part A, and introduces glyph items");Capture("slice-390-glyphs-a.png");});
