...
(async()=>{
  ...
  await action('seal');
  check(await recovery.evaluate(()=>window.ascendantDial.snapshot().destination==='Framed: Aries' && window.ascendantDial.snapshot().phase.includes('Hint level 1')),'first browser rejection stays in place at Level 1');
  await action('seal');await action('count');
  check(await recovery.evaluate(()=>window.ascendantDial.snapshot().destination==='Framed: Aries' && window.ascendantDial.snapshot().phase.includes('Hint level 2')),'second browser rejection and Count preserve Level 2');
  await recovery.screenshot({path:path.join(out,'390-rejected.png')});
  await action('seal');await active('Leo');
  check(await recovery.evaluate(()=>window.ascendantDial.snapshot().phase.includes('Hint level 0')),'Level 3 demo resets to a fresh Level 0 problem');
  for(let n=0;n<3;n++)await action('seal');await active('Sagittarius');
  for(let n=0;n<3;n++)await action('seal');
  await recovery.waitForFunction(()=>window.ascendantDial.snapshot().phase.includes('Later review pending'),{},{timeout:15000});
