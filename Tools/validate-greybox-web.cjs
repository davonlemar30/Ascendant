...
(async()=>{
  ...
  for(const viewport of [{width:390,height:844},{width:360,height:800}]){
    ...
    { const g=await state(); check(g.avatarAt==='grid'&&g.gridStarted && g.gridTiles.length===12 && g.gridCells.length===12 && g.gridCells.every(c=>c.endsWith(': empty')) && g.gridTiles.every(t=>t.endsWith(', not seated')) && g.caspar.startsWith('Four elements, three kinds') && g.keys===2,'the room button walks to the table and opens it: twelve tiles, twelve empty cells, the intro line at '+viewport.width); }
    ...
    check((await state()).caspar==='Not that cell. Taurus is an Earth sign.' && (await state()).canGridAsk && (await state()).gridLocked && (await state()).gridCells[CELL(8)].endsWith('not that one'),'a wrong row nudges with the element, marks the cell, and offers Ask Caspar at '+viewport.width);
    ...
    await page.waitForFunction(()=>window.ascendantDial.snapshot().gridHintLevel===1,{},{timeout:5000});
    check((await state()).caspar==='Not that cell. Gemini is mutable.','a wrong column nudges with the kind at '+viewport.width);
    await semantic('grid-ask');await page.waitForFunction(()=>window.ascendantDial.snapshot().gridHintLevel===2,{},{timeout:5000});
    check((await state()).caspar.startsWith('Gemini is Air; it is mutable.') && !(await state()).canGridAsk && events.some(e=>e.event_name==='hint_asked'&&e.requested_relationship==='cell_of_sign'),'Ask Caspar states the rule once at '+viewport.width);
    ...
    await semantic('enter-wing');await page.waitForFunction(()=>window.ascendantDial.snapshot().screen==='wingroom'&&!window.ascendantDial.snapshot().busy,{},{timeout:15000});
    check((await state()).caspar.includes('part seated'),'the room says the table waits, part seated, at '+viewport.width);
    ...
    await settled();
    check((await state()).gridPlaced===6 && (await state()).caspar.includes('Virgo is seated') && !events.filter(e=>e.event_name==='grid_placed')[5].evidence_eligible,'three wrong cells hand the sign to Caspar, who seats it without evidence at '+viewport.width);
  ...
  await action('ask-caspar');await recovery.waitForFunction(()=>window.ascendantDial.snapshot().hintLevel===2&&window.ascendantDial.snapshot().message.includes('Find the next elemental sign after'));
